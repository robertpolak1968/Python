# GreconQ-WX.py by Robert Polak v.0.97
...
def helpmy (event):
    print "GreconQ - reduce quantity at Cygnum Ltd"
    print 'You can change name of file to reduce quantity'
    print 'For example:'
    print 'Location of file is: C:\Grecon\\'
    print 'Flie name is: 1-38x140.txt'
    print "Please type: C:\Grecon\\1-38x140.txt"
    print 'Tip : copy text path from windows files explorer '
    print 'Tip : Simply change path file name to do next file.(1-38x89.txt,1-38x140.txt, 1-38x195.txt, ... )'
    print 'Botton LOAD: load and display indicated file'
    print 'Botton SAVE: save and display indicated file'
    print 'Botton RUN: load, reduce quantity, save and display indicated file'
    print ''
    print 'Grecon-WX.py by Robert Polak v.0.97'
def load(event):
...
def run(event):
    ...
    printw ('Number of rows of source file : ') 
    printw (iloscwierszy)
    printw ('')
...
#for i in range (0, len(ms)) : print ms[i]
    ...
    printw  ('Number rows of destination file : ')
    rows = len(msfinal)+1
    printw (rows)
    printw ('')
...
app = wx.App()
win = wx.Frame(None, title="GreconQ. By Robert Polak V.0.97 ", size=(1200, 800))
bkg = wx.Panel(win)
...
helpmyButton = wx.Button(bkg, label='Help')
helpmyButton.Bind(wx.EVT_BUTTON, helpmy)
...
hbox.Add(helpmyButton, proportion=0, flag=wx.LEFT, border=5)
